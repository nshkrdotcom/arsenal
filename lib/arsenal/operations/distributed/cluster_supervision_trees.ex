defmodule Arsenal.Operations.Distributed.ClusterSupervisionTrees do
  ...
  def get_local_supervision_trees(params) do
    try do
      # TODO: Replace with Arsenal equivalent when available
      supervisors = list_supervisors_placeholder()
  ...
  # TODO: Replace with Arsenal equivalent
  defp list_supervisors_placeholder, do: []
  ...
  defp guess_application_from_name(name) when is_atom(name) do
    ...
    cond do
      String.contains?(name_str, "Arsenal") -> :arsenal
      String.contains?(name_str, "Phoenix") -> :phoenix
      String.contains?(name_str, "Elixir") -> :elixir
      String.contains?(name_str, "Sandbox") -> :sandbox
      true -> :unknown
    end
  end
  ...
  defp get_supervisor_strategy(supervisor) do
    case supervisor.pid do
      ...
      pid_str ->
        ...
        try do
          case to_pid_placeholder(pid_str) do
  ...
  # TODO: Replace with Arsenal equivalent
  defp to_pid_placeholder(_pid_str), do: {:error, :not_implemented}
  ...
  defp get_supervisor_children(supervisor_pid, include_process_details, max_depth, current_depth) do
    try do
      # TODO: Replace with Arsenal equivalent
      case get_supervision_tree_placeholder(supervisor_pid) do
        {:ok, tree} ->
          tree.children
          |> Enum.map(fn child ->
            base_child = %{
  ...
  # TODO: Replace with Arsenal equivalent
  defp get_supervision_tree_placeholder(_supervisor_pid), do: {:error, :not_implemented}
  ...
  defp get_cluster_health_status do
    try do
      # TODO: Replace with Arsenal equivalent when available
      get_partition_status_placeholder()
    rescue
      _ -> :unknown
    catch
      :exit, _ -> :unknown
  ...
  # TODO: Replace with Arsenal equivalent
  defp get_partition_status_placeholder, do: :unknown
end